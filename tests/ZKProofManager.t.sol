...
contract ZKProofManagerTest is Test {
    ...
    function setUp() public {
        ...
        ZKProofManager.ProofTypeConfig[] memory configs = new ZKProofManager.ProofTypeConfig[](4);
        configs[0] = ZKProofManager.ProofTypeConfig({name: "age_gte", verifier: address(ageVerifier), active: true});
        configs[1] = ZKProofManager.ProofTypeConfig({name: "age_lte", verifier: address(ageMaxVerifier), active: true});
        configs[2] = ZKProofManager.ProofTypeConfig({name: "attr_eq", verifier: address(attrVerifier), active: true});
        configs[3] =
            ZKProofManager.ProofTypeConfig({name: "income_gte", verifier: address(incomeVerifier), active: true});

        manager = new ZKProofManager(address(this), configs);
        manager.anchorRoot(ROOT);
    }

    function _dummyProof() private pure returns (IZKProofManager.Proof memory proof, uint256[] memory signals) {
    ...
    function testInitialProofTypesRegistered() public view {
        IZKProofManager.ProofTypeInfo[] memory allTypes = manager.getAllProofTypes();
        assertEq(allTypes.length, 4, "type count");
        assertEq(allTypes[uint256(ZkTypes.ageGte())].verifier, address(ageVerifier), "age_gte verifier");
    }
    ...
    function testVerifyAgeProofMarksNullifier() public {
        (IZKProofManager.Proof memory proof, uint256[] memory signals) = _dummyProof();
    ...
    function testVerifyProofRevertsForInactiveType() public {
        manager.updateProofType(ZkTypes.attrEquals(), address(attrVerifier), false);
        (IZKProofManager.Proof memory proof, uint256[] memory signals) = _dummyProof();
    ...
    function testVerifyProofRevertsForUnanchoredRoot() public {
        ...
        manager.verifyIncomeProof(keccak256("bad"), keccak256("n"), proof, signals);
    }
    ...
    function testAddProofTypeViaAdmin() public {
        ...
        uint256 typeId = manager.addProofType("memory_circuit", address(memoryVerifier), true);
        IZKProofManager.ProofTypeInfo memory info = manager.getProofType(typeId);
