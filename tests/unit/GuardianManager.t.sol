...
contract GuardianManagerTest is Test {
    ...
    function testSetGuardiansRequiresExactCount() public {
        address[] memory guardians = new address[](2);
        guardians[0] = G1;
        guardians[1] = G2;

        vm.prank(OWNER);
        vm.expectRevert(GuardianManager.InvalidGuardianCount.selector);
        manager.setGuardians(OWNER, guardians);
    }

    function testAddGuardianRejectsOwnerAddress() public {
        vm.prank(OWNER);
        vm.expectRevert(GuardianManager.InvalidAddress.selector);
        manager.addGuardian(OWNER, OWNER);
    }
}
