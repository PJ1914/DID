...
contract SessionKeyManagerTest is Test {
    ...
    function testAddSessionKeyStoresConfig() public {
        ...
        assertTrue(
            manager.isSessionKeyValid(WALLET, SESSION_KEY),
            "session key valid"
        );

        SessionKeyManager.SessionKeyConfig memory config = manager
            .getSessionKeyConfig(WALLET, SESSION_KEY);
        ...
        assertEq(
            keccak256(bytes(config.allowedFunctions[0])),
            keccak256(bytes("transfer")),
            "allowed function"
        );
        assertEq(
            config.allowedContracts[0],
            address(0xABC),
            "allowed contract"
        );
    }
    ...
    function testAddSessionKeyRevertsOnInvalidAddresses() public {
