...
contract OrganizationManagerTest is Test {
    ...
    function testRegisterOrganizationInitializesRecord() public {
        ...
        OrganizationTypes.Organization memory org = manager.getOrganization(
            orgId
        );
        ...
        assertEq(
            uint8(org.status),
            uint8(OrganizationTypes.OrganizationStatus.Pending),
            "status"
        );
        assertTrue(
            manager.hasOrganizationRole(orgId, ADMIN, Roles.ORGANIZATION_ADMIN),
            "admin role"
        );
    }
    ...
    function testSetOrganizationStatusRequiresSystemAdmin() public {
        ...
        assertEq(
            uint8(org.status),
            uint8(OrganizationTypes.OrganizationStatus.Active),
            "status"
        );
    }
    ...
    function testAssignAndRevokeRole() public {
        ...
        assertTrue(
            manager.hasOrganizationRole(
                orgId,
                MEMBER,
                Roles.VERIFICATION_PROVIDER
            ),
            "role assigned"
        );
