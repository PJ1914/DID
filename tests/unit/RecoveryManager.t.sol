...
contract RecoveryManagerTest is Test {
    ...
    function testRecoveryFlowRequiresMajority() public {
        vm.prank(guardian1);
        uint256 recoveryId = recoveryManager.requestRecovery(
            wallet,
            newOwner,
            "lost device",
            3 days,
        ...
        IRecoveryManager.RecoveryDetails memory details = recoveryManager
            .getRecovery(recoveryId);
        assertEq(details.confirmations, 1, "initial confirmations");
        assertEq(
            uint256(details.status),
            uint256(IRecoveryManager.RecoveryStatus.Pending),
            "pending status"
        );
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                RecoveryManager.DelayNotElapsed.selector,
                recoveryId,
                details.executeAfter
            )
        );
        recoveryManager.executeRecovery(recoveryId, wallet, owner);

        vm.warp(block.timestamp + 3 days + 1);
        address resultOwner = recoveryManager.executeRecovery(
            recoveryId,
            wallet,
            owner
        );
        ...
        assertEq(
            uint256(details.status),
            uint256(IRecoveryManager.RecoveryStatus.Executed),
            "executed status"
        );
    }
    ...
    function testCannotDuplicateConfirmations() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                RecoveryManager.AlreadyConfirmed.selector,
                recoveryId,
                guardian2
            )
        );
    ...
    function testOwnerCanCancelRecovery() public {
        ...
        assertEq(
            uint256(details.status),
            uint256(IRecoveryManager.RecoveryStatus.Cancelled),
            "cancelled"
        );
    }

    function testRequestRecoveryRequiresInitiatingGuardian() public {
        vm.expectRevert(RecoveryManager.Unauthorized.selector);
        vm.prank(address(0x9999));
        recoveryManager.requestRecovery(
            wallet,
            newOwner,
            "phishing",
            1 days,
    ...
    function testConfirmRecoveryRejectsNonGuardian() public {
        vm.prank(guardian1);
        uint256 recoveryId = recoveryManager.requestRecovery(
            wallet,
            newOwner,
