...
contract AlchemyGasManagerTest is Test {
    ...
    function testOnboardingEligibility() public view {
        AlchemyGasManager.Decision memory decision = manager.shouldSponsorGas(
            bytes32(0),
            onboardingUser,
            100_000
        );
    ...
    function testRecordOnboardingUsageUpdatesState() public {
        ...
        assertEq(
            manager.dailyGasUsed(onboardingUser, day),
            150_000,
            "daily usage"
        );
        assertEq(
            manager.monthlyGasUsed(onboardingUser, month),
            150_000,
            "monthly usage"
        );
        assertEq(
            manager.totalGasSponsored(onboardingUser),
            150_000,
            "total usage"
        );
        assertGt(
            manager.onboardingStart(onboardingUser),
            0,
            "onboarding start"
        );
    }
    ...
    function testPaymasterDataEncoding() public {
        ...
        (
            ...
            bytes32 dappId
                ...
                data,
                (address, string, string, bytes32, uint256, bool, bytes32)
            );

        assertEq(paymaster, address(0xFEED), "paymaster");
        assertEq(
            keccak256(bytes(policy)),
            keccak256(bytes("policy")),
            "policy id"
        );
    ...
    function testWhitelistEnforcement() public {
        ...
        (bool eligible, , , , ) = manager.getPaymasterData(
            bytes32(0),
            onboardingUser,
            100_000,
            bytes32("REJECT")
        ...
        (bool allowed, , , , ) = manager.getPaymasterData(
            bytes32(0),
            onboardingUser,
            100_000,
            bytes32("APPROVED")
    ...
    function testUpdateAlchemyConfigAppliesChanges() public {
        manager.updateAlchemyConfig(
            "newPolicy",
            "newApp",
            address(0xDEAD),
            3_000_000,
        ...
        assertEq(keccak256(bytes(app)), keccak256("newApp"), "app");
        assertEq(paymaster, address(0xDEAD), "paymaster");
        assertEq(maxGas, 3_000_000, "max gas");
        assertTrue(backend, "backend flag");
    }

    function testSetOnboardingSettingsUpdatesState() public {
        manager.setOnboardingSettings(500_000, 3 days);
        assertEq(manager.onboardingAllowance(), 500_000, "allowance");
        assertEq(manager.onboardingPeriod(), 3 days, "period");
    }

    function testCustomRuleRegistrationAndLookup() public {
        bytes32 customRule = keccak256("CUSTOM_RULE");
        AlchemyGasManager.SponsorshipRule memory rule = AlchemyGasManager
            .SponsorshipRule({
            active: true,
            minTrustScore: 1000,
            maxDailyGas: 1_000_000,
            maxMonthlyGas: 5_000_000,
            maxPerTxGas: 500_000
        });

        manager.updateSponsorshipRule(customRule, rule);
        (
            bool active,
            uint256 minTrust,
            uint256 maxDaily,
            uint256 maxMonthly,
            uint256 maxPerTx
        ) = manager.rules(customRule);
        assertTrue(active, "rule active");
        assertEq(minTrust, 1000, "trust threshold");
        assertEq(maxDaily, 1_000_000, "max daily");
        assertEq(maxMonthly, 5_000_000, "max monthly");
        assertEq(maxPerTx, 500_000, "max per tx");

        bytes32[] memory ids = manager.getRuleIds();
        bool found;
        ...
        assertTrue(found, "rule id present");
    }

    function testRecordGasSponsorshipForStandardRule() public {
        bytes32 identityId = keccak256("user");
        trustScore.setScore(identityId, 900, "seed");
        ...
        assertEq(manager.totalGasSponsored(highTrustUser), 100_000);
        assertEq(trustScore.getScore(identityId), 901, "trust incremented");
    }

    function testRecordGasSponsorshipRevertsForInactiveRule() public {
        bytes32 customRule = keccak256("INACTIVE_RULE");
        manager.updateSponsorshipRule(
            customRule,
            AlchemyGasManager.SponsorshipRule({
                active: false,
                minTrustScore: 0,
                maxDailyGas: 0,
                maxMonthlyGas: 0,
        ...
        vm.expectRevert(AlchemyGasManager.InvalidRuleId.selector);
        vm.prank(operator);
        manager.recordGasSponsorship(
            bytes32(0),
            onboardingUser,
