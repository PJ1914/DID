...
contract IdentityRegistryTest is Test {
    ...
    function testRegisterIdentityStoresProfile() public {
        ...
        IdentityTypes.IdentityProfile memory profile = registry.getIdentity(identityId);
        assertEq(profile.owner, USER, "owner");
        assertEq(profile.metadataURI, INITIAL_URI, "metadata");
        assertEq(uint8(profile.status), uint8(IdentityTypes.IdentityStatus.Pending), "status");
