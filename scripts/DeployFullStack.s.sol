...
contract DeployFullStack is Script {
    ...
    function run() external {
        ...
        addressesOut.disputeResolution = address(disputeResolution);
        ...
        address walletImplementation = vm.envOr(
        ...
        if (walletImplementation == address(0)) {
            ...
            if (deployMock) {
                walletImplementation = address(
                    new BasicWalletAccount(address(0), new address[](0), 0)
                );
            } else {
                revert(
                    "WALLET_IMPLEMENTATION env required or set DEPLOY_MOCK_WALLET=true"
                );
            }
        }
        ...
        address entryPoint = vm.envOr(
            "ENTRY_POINT",
            address(0x0000000071727de22e5E9D8bAF0eDAcb0f3F0EAB)
        );
        ...
        ISessionKeyManager sessionKeyManager = walletManager
            .sessionKeyManager();
    ...
    function _logSummary() internal view {
        ...
        console2.log("DisputeResolution", addressesOut.disputeResolution);
    ...
    function _writeDeploymentJson() internal {
        ...
        vm.serializeAddress(
            root,
            "verificationLogger",
            addressesOut.verificationLogger
        );
        vm.serializeAddress(root, "trustScore", addressesOut.trustScore);
        vm.serializeAddress(
            root,
            "identityRegistry",
            addressesOut.identityRegistry
        );
        vm.serializeAddress(
            root,
            "verificationManager",
            addressesOut.verificationManager
        );
        vm.serializeAddress(
            root,
            "organizationManager",
            addressesOut.organizationManager
        );
        vm.serializeAddress(
            root,
            "disputeResolution",
            addressesOut.disputeResolution
        );
        vm.serializeAddress(
            root,
            "guardianManager",
            addressesOut.guardianManager
        );
        vm.serializeAddress(
            root,
            "alchemyGasManager",
            addressesOut.alchemyGasManager
        );
        vm.serializeAddress(root, "walletManager", addressesOut.walletManager);
        vm.serializeAddress(
            root,
            "recoveryManager",
            addressesOut.recoveryManager
        );
        vm.serializeAddress(
            root,
            "sessionKeyManager",
            addressesOut.sessionKeyManager
        );
        vm.serializeAddress(
            root,
            "zkProofManager",
            addressesOut.zkProofManager
        );
        vm.serializeAddress(
            root,
            "ageGteVerifier",
            addressesOut.ageGteVerifier
        );
        vm.serializeAddress(
            root,
            "ageLteVerifier",
            addressesOut.ageLteVerifier
        );
        vm.serializeAddress(
            root,
            "attrEqualsVerifier",
            addressesOut.attrEqualsVerifier
        );
        string memory json = vm.serializeAddress(
            root,
            "incomeGteVerifier",
            addressesOut.incomeGteVerifier
        );
